...
function getCookieEntryHTML() as String
  ...
  html = html + "<body><div class='container'><h1>Hydravion Cookie Entry</h1><div class='instructions'><p>Enter your <strong>sails.sid</strong> cookie from floatplane.com</p><p style='font-size:12px;color:#aaa;margin-top:10px'><strong>How to get cookie:</strong><br>1. Login to <a href='https://www.floatplane.com' target='_blank' style='color:#4a9eff'>floatplane.com</a><br>2. Open DevTools (F12)<br>3. Go to Application (Chrome) or Storage (Firefox) > Cookies > floatplane.com<br>4. Copy the value of the <strong>sails.sid</strong> cookie</p></div>"
  ...
  html = html + "const data=await response.json();if(data.success){showStatus('Cookie sent successfully! Return to your Roku device and enjoy!',false);btn.textContent='Sent!'}else{throw new Error('Failed to send cookie')}}"
